...
class Engine(interfaces.AbstractEngine):
  ...
  def create_conversation(
    ...
    del filter_channel_content_from_kv_cache  # Unused in this implementation
  ...
  def eos_token_ids(self) -> list[list[int]]:
    ...
    try:
      ...
      for i in range(num):
        ...
        # Note: litert_lm_token_unions_get_token_at returns a borrowed pointer.
        # But _parse_token_union deletes it. This is a problem.
        # I should make a version that doesn't delete.
        val = self._parse_token_union_no_delete(u_ptr)
        if isinstance(val, int):
          all_ids.append([val])
  ...
  def _parse_token_union_no_delete(self, union_ptr):
    if not union_ptr:
      return None
    u_type = self._lib.litert_lm_token_union_get_type(union_ptr)
    if u_type == 0:  # String
      s = self._lib.litert_lm_token_union_get_string(union_ptr)
      return s.decode("utf-8") if s else None
    elif u_type == 1:  # Ids
      ids_ptr = ctypes.POINTER(ctypes.c_int)()
      num_ids = ctypes.c_size_t()
      if (
          self._lib.litert_lm_token_union_get_ids(
