...
TEST(AudioExecutorSettingsTest, GetAndSetAudioStreamingEnabled) {
  ASSERT_OK_AND_ASSIGN(ModelAssets model_assets, ModelAssets::Create(""));
  ASSERT_OK_AND_ASSIGN(AudioExecutorSettings settings,
                       AudioExecutorSettings::CreateDefault(
                           model_assets, 10, Backend::GPU_ARTISAN));
  EXPECT_FALSE(settings.GetAudioStreamingEnabled());
  settings.SetAudioStreamingEnabled(true);
  EXPECT_TRUE(settings.GetAudioStreamingEnabled());
}
